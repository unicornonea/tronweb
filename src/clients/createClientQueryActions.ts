...
interface CreateClientQueryActionsOptions {
    readonly chain?: Chain;
    readonly tronWeb: TronWeb;
    readonly trx: PublicClientQueryTrx;
}
...
function resolveCallerAddress(account: string | undefined, tronWeb: TronWeb): string {
    if (typeof account === 'string' && account.length > 0) return account;
    if (typeof tronWeb.defaultAddress.hex === 'string' && tronWeb.defaultAddress.hex.length > 0) return tronWeb.defaultAddress.hex;
...
function extractConstantResultData(tronWeb: TronWeb, transaction: TransactionWrapper): Hex | undefined {
...
function resolveBlockTransactionCountInput(
...
): Parameters<PublicClientQueryTrx['getBlockTransactionCount']>[0] {
    ...
    return entries[0][1] as Parameters<PublicClientQueryTrx['getBlockTransactionCount']>[0];
}
...
): Parameters<PublicClientQueryTrx['getTransactionInfo']>[0] {
...
export function createClientQueryActions({ chain, tronWeb, trx }: CreateClientQueryActionsOptions): PublicClientActions {
    return {
        call: async ({ account, data, to, value }: CallParameters): Promise<CallReturnType> => {
            const transaction = await tronWeb.transactionBuilder.triggerConstantContract(
                ...
                [],
                resolveCallerAddress(account, tronWeb)
            );

            return {
                data: extractConstantResultData(tronWeb, transaction),
            };
        },
        ...
        getContractEvents: async <Abi extends ContractAbiInterface>({
            ...
            const response = await tronWeb.event.getEventsByContractAddress(address, {
                ...options,
        ...
        getBlockTransactionCount: (input) => trx.getBlockTransactionCount(resolveBlockTransactionCountInput(input)),
        getChainId: async () => {
        ...
        getLogs: async ({ address, ...options }: GetLogsParameters): Promise<GetLogsReturnType> => {
            const response = await tronWeb.event.getEventsByContractAddress(address, options);
        ...
        getTransactionReceipt: async (input) => trx.getTransactionInfo(resolveTransactionHashInput(input, 'getTransactionReceipt')),
        ...
        >({
            ...
            return normalizeReadContractOutput(
                fragment,
                extractConstantResultData(tronWeb, transaction)
